...
int main()
{
    ...
    vector<vector<long long>> dist(3, vector<long long>(n + 1, LONG_LONG_MAX));
    dist[0][1] = 0;
    pq.push({0, {1, 0}});
    while (!pq.empty())
    {
        ...
        for (auto it : adj[node])
        {
            ...
            if (state != 2 && dis + w / 2 < dist[state+1][v])
            {
                dist[state+1][v] = dis + w / 2;
                pq.push({dist[state+1][v], {v, state+1}});
            }
        }
    }
